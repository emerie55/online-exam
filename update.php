...
if(isset($_SESSION['key'])){
  if(@$_GET['demail2'] && $_SESSION['key']=='prasanth123') {
  $demail2=@$_GET['demail2'];
  
  $result = mysqli_query($con,"DELETE FROM pi_n_code WHERE pincode='$demail2' ") or die('Error');
...
//remove quiz Admin
if(isset($_SESSION['key'])){
  if(@$_GET['q']== 'rmquiz1' && $_SESSION['key']=='prasanth123') {
  $eid=@$_GET['eid'];
  $result = mysqli_query($con,"SELECT * FROM questions WHERE eid='$eid'") or die('Error');
  while($row = mysqli_fetch_array($result)) {
    $qid = $row['qid'];
  ...
  header("location:headdash.php?q=51");
  }
  }
...
//add quiz admin
if(isset($_SESSION['key'])){
  if(@$_GET['q']== 'addquiz1' && $_SESSION['key']=='prasanth123') {
  $name = $_POST['name'];
  $name= ucwords(strtolower($name));
  $total = $_POST['total'];
  $sahi = $_POST['right'];
  ...
  header("location:headdash.php?q=41&step=2&eid=$id&n=$total");
  }
  }
  
  //add question admin
  if(isset($_SESSION['key'])){
  if(@$_GET['q']== 'addqns1' && $_SESSION['key']=='prasanth123') {
  $n=@$_GET['n'];
  $eid=@$_GET['eid'];
  $ch=@$_GET['ch'];
